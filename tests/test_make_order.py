...
class TestMakeOrder:
    ...
    def test_upper_order_button(self,driver):
        ...
        assert '/order' in main_page.get_page(), 'Кнопка ведёт на другую страницу'
    ...
    def test_make_order(self, driver, customer):
        ...
        assert data.MAIN_PAGE in order_page.get_page(), 'Нажатие на логотип "Самоката" ведёт на другую страницу'
        order_page.click_logo_yandex()
        order_page.switch_window()
        assert data.DZEN_PAGE in order_page.get_page(), 'главная страница Дзен не открылась'
